...
void unhide_bars(void) {
    ...
    SLIST_FOREACH(walk, outputs, slist) {
        ...
        else values[1] = walk->rect.y + walk->rect.h - bar_height- (config.separator_line?1:0);
    ...
    SLIST_FOREACH(walk, outputs, slist) {
        ...
        if (walk->bar == XCB_NONE) {
            ...
            xcb_void_cookie_t win_cookie = xcb_create_window_checked(xcb_connection,
                                                                     ...
                                                                     walk->rect.w, bar_height+(config.separator_line?1:0),
                                                                     0,
                                                                     XCB_WINDOW_CLASS_INPUT_OUTPUT,
                                                                     root_screen->root_visual,
                                                                     mask,
            ...
            mask = XCB_CONFIG_WINDOW_X |
                   ...
                   XCB_CONFIG_WINDOW_STACK_MODE;
            values[0] = walk->rect.x;
            values[1] = walk->rect.y + walk->rect.h - bar_height-(config.separator_line?1:0);
            values[2] = walk->rect.w;
            values[3] = bar_height+(config.separator_line?1:0);
            values[4] = XCB_STACK_MODE_ABOVE;
...
void draw_bars(bool unhide) {
    ...
    SLIST_FOREACH(outputs_walk, outputs, slist) {
        ...
        xcb_change_gc(xcb_connection,
                      ...
                      XCB_GC_FOREGROUND,
                      &color);
        xcb_rectangle_t rect = { 0, 0, outputs_walk->rect.w, bar_height +(config.separator_line?1:0)};
